...
| Privileges | E081 | **partial** | [GRANT](statements/grant) and [REVOKE](statements/revoke) exist, but not in the standard's shape: they confer a *role* (`READER`/`WRITER`/`OWNER`) on a workspace, collection or dataset, not the standard's per-privilege grants (`SELECT`, `INSERT`, `UPDATE`, `DELETE`, `REFERENCES`), and there is no `WITH GRANT OPTION` or `PUBLIC`. [SHOW GRANTS](statements/show-grants) inspects what the current session holds; [SHOW GRANTS ON](statements/show-grants-on) and [SHOW EFFECTIVE GRANTS ON](statements/show-effective-grants-on) inspect an object. |

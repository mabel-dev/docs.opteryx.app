...
- DDL: `CREATE` / `DROP` / `TRUNCATE` / `ALTER TABLE`, `CREATE` / `ALTER` / `DROP VIEW`, `CREATE` / `DROP MATERIALIZED VIEW`, `DROP TRIGGER`, `DROP COLLECTION`, `ALTER WORKSPACE`, `COMMENT ON`, `ANALYZE TABLE`

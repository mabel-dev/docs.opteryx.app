...
| `is_current` | `true` for the snapshot a plain `SELECT` reads today; `false` for the rest. After a [rollback](alter-table#rollback-to-version) this is **not** the newest row |
...
can write: `VERSION AS OF current` reads exactly like `VERSION AS OF <any tag>`. `current`
and `previous` are reserved, so no real tag can take either name.

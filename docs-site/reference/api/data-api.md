...
Base URL: https://data.opteryx.app

## Overview

The Data service accepts SQL statements for execution and exposes job status and results endpoints. Incoming requests must present a valid Bearer JWT verified against the Authentication service JWKS.
...
Authentication
- Bearer JWT required for protected endpoints. Tokens are validated against JWKS from the Authentication service. `DATA_AUDIENCE` expected in `aud` claim.

...
This page explains how to sign in to Opteryx using the web UI. Opteryx supports OAuth sign-in with Google, Microsoft and GitHub. API tokens and personal access tokens are managed from the Opteryx Studio settings pages.
...
- OAuth is used for interactive access only. Tokens and API keys are created and managed from the **Settings** → **API Tokens** page in Opteryx Studio.
...
- If sign-in fails, check your browser's popup blocker and ensure the provider is allowed to redirect to the Opteryx domain.

...
Any of those points can be read directly. `PREVIOUS` is the snapshot before the current one, so this is the table as it stood before the `DELETE` — with Pluto still in it:

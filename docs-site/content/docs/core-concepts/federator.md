...
`federator` gets its access the exact same way a person does - a role on a resource pattern via the [Policy API](/docs/reference/api/policy-api) (see [Security & Permissions](/docs/core-concepts/access-and-permissions)). That means it shows up as an ordinary row in a workspace's access list (`federator - writer - via collection sales`, for example), and its grants can be reviewed, narrowed, or revoked from Manage Access exactly like anyone else's.
